 ...
 * red indicators on dates where excuses were rejected/declined by ,
 * emerald check indicators on dates where excuses were approved by ,
  ...
  return (
    <div style={{ position: 'relative', display: fullWidth ? 'block' : 'inline-block', width: fullWidth ? '100%' : 'auto' }}>
      ...
      <button
        ...
        title={disabled ? 'Date selection disabled' : isSelectedRejected ? 'Excuse Request Rejected by  — Re-filing Not Allowed' : isSelectedApproved ? 'Excuse Approved by  — Formation Excused' : isSelectedSubmitted ? 'Excuse Request Already Submitted — Pending Admin Review' : 'Open interactive formation calendar'}
      >
              ...
              {Array.from({ length: daysInCalMonth }).map((_, idx) => {
                ...
                if (isRecorded || isApproved || isSubmitted || isRejected) {
                  ...
                  const dayTitle = isRejected
                    ? `Excuse Rejected by  (Cannot Re-file): ${formatHumanDate(dayKey)}`
                    : isApproved
                      ? `Excuse Approved by  (EXCUSED): ${formatHumanDate(dayKey)}`
                      : isSubmitted
